...
def test_generate_system_metadata():
    ...
    with httmock.HTTMock(mock_dataone_formats):
        ...
        sys_meta = metadata.generate_system_metadata(pid, name, format_id,
                                                     size, md5, rights_holder)

        assert sys_meta.identifier.value() == pid
        assert sys_meta.formatId == format_id
        assert sys_meta.size == size
        assert sys_meta.submitter == rights_holder
        assert sys_meta.rightsHolder == rights_holder
        assert sys_meta.checksum == dataoneTypes.checksum(str(md5))
